...
const navItems = [
  { label: 'Dashboard',       icon: '\u229e', badge: null,  active: true },
  { label: 'My Profile',      icon: '\u{1f464}', badge: null },
  { label: 'My Orders',       icon: '\u{1f4e6}', badge: null },
  { label: 'My Cart',         icon: '\u{1f6d2}', badge: 3   },
  { label: 'Wishlist',        icon: '\u2665', badge: 7   },
  { label: 'Addresses',       icon: '\u{1f4cd}', badge: null },
  { label: 'Payment Methods', icon: '\u{1f4b3}', badge: null },
  { label: 'My Reviews',      icon: '\u2605', badge: null },
  { label: 'Notifications',   icon: '\u{1f514}', badge: null },
  { label: 'Support / Help',  icon: '\u{1f4ac}', badge: null }
]
...
export default function UserDashboard() {
  ...
  function handleNav(label) {
    switch (label) {
      case 'Dashboard': return navigate('/user-dashboard')
      case 'My Profile': return navigate('/profile')
      case 'My Orders': return navigate('/user-orders')
      case 'My Cart': return navigate('/cart')
      case 'Wishlist': return navigate('/wishlist')
      case 'Addresses': return navigate('/addresses')
      case 'Payment Methods': return navigate('/payment-methods')
      case 'My Reviews': return navigate('/reviews')
      case 'Notifications': return navigate('/notifications')
      case 'Support / Help': return navigate('/support')
      default: return
    }
  }
  ...
  return (
    <div className="ud-root">
      ...
      <header className="ud-topnav">
        ...
        <button className="ud-hamburger" aria-label="menu">
          <span /><span /><span />
        </button>
      ...
      <div className="ud-body">
        ...
        <aside className="ud-sidebar">
          <nav className="ud-nav">
            {navItems.map(item => {
              const badgeValue = item.label === 'My Cart'
                ? cartCount
                : item.label === 'Wishlist'
              ...
              return (
                <button
                  key={item.label}
                  className={`ud-nav-item${item.active ? ' ud-nav-active' : ''}`}
                  type="button"
                ...
                >
                  ...
                  {badgeValue != null && (
                    <span className="ud-nav-badge">{badgeValue}</span>
                  )}
                </button>
              )
            })}
